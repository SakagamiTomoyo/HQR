...
class ProjectionAggregating(nn.Module):
    def __init__(self, dim, cen, aggr='mlp'):
        ...
        self.attn_layer_1 = nn.Linear(self.dim * 4, self.dim)
        self.attn_layer_2 = nn.Linear(self.dim, self.dim)

        self.offset_layer_1 = nn.Linear(self.dim * 3, self.dim)
        self.offset_layer_2 = nn.Linear(self.dim, self.dim)
    ...
    def forward(self, query_emb, offset_emb, refer_embs,
        ...
        layer_act_1 = F.relu(self.attn_layer_1(
            torch.cat([refer_r,
                       refer_r_offset,
                       query_r.unsqueeze(1).repeat(1, refer_r.shape[1], 1),
                       query_r_offset.unsqueeze(1).repeat(1, refer_r.shape[1], 1)], dim=-1
                      )
        ))
        ...
        # const = 2.5  # training 10
        #
        # final_refer_emb = final_refer_emb / (1e-9 + ref_norm / query_norm * const)

        final_query_emb = query_emb + final_refer_emb

        final_refer_offset = self.offset_layer_2(F.relu(self.offset_layer_1(
            torch.cat([query_emb, offset_emb, final_query_emb], dim=-1)
        )))

        ref_norm = final_refer_offset.norm(p=1, dim=-1).unsqueeze(1)
        query_norm = offset_emb.norm(p=1, dim=-1).unsqueeze(1)
        const = 1e9  # training 10

        final_refer_offset = final_refer_offset / (1e-9 + ref_norm / query_norm * const)

        final_offset_emb = offset_emb + final_refer_offset
...
class IDN_Q2B_24(nn.Module):
    ...
    def forward(self, chosen_queries, query_type, sample_limit, subgraph, chosen_answers, query_name_dict,
        ...
        if query_name_dict[query_type] == '1p':
            ...
            offset_emb_1 = self.func(r_offset[rel_2])
            offset_emb_2 = self.func(r_offset[rel_2])
