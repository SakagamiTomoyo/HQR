...
class ProjectionAggregating(nn.Module):
    def __init__(self, dim, aggr='mlp'):
        ...
        # self.zoom_layer_1 = nn.Linear(self.dim * 2, self.dim)
        # self.zoom_layer_2 = nn.Linear(self.dim, 1)

        nn.init.xavier_uniform_(self.attn_layer_1.weight)
        nn.init.xavier_uniform_(self.attn_layer_2.weight)

        # nn.init.xavier_uniform_(self.zoom_layer_1.weight)
        # nn.init.xavier_uniform_(self.zoom_layer_2.weight)

    def forward(self, query_emb, refer_embs, query_r=None, refer_r=None, start_embs=None):
        # return query_emb + torch.mean(refer_embs, dim=-2)
        ...
        # zoom = torch.sigmoid(
        #     self.zoom_layer_2(torch.tanh(self.zoom_layer_1(torch.cat([query_emb, final_refer_emb], dim=-1))))
        # )
        #
        # # # 可以再乘以alpha
        # final_query_emb = query_emb + final_refer_emb * zoom
        #
        return final_query_emb


class IDN_GQE_21(nn.Module):
    def __init__(self, graph, args):
        super(IDN_GQE_21, self).__init__()
    ...
    def forward(self, chosen_queries, query_type, sample_limit, subgraph, chosen_answers, query_name_dict,
        ...
        if not evaluating:
            ...
            distance = target_embeds - query_emb
            scores = self.gamma - torch.norm(distance, p=1, dim=-1)

            neg_embeds = self.entity_embedding[torch.tensor(neg_answers)]
            neg_distance = neg_embeds - query_emb.unsqueeze(1)
            neg_scores = self.gamma - torch.norm(neg_distance, p=1, dim=-1)
            ...
            if 'u' in query_name_dict[query_type]:
                ...
                for i in range(batch_size):
                    distance = target_embeds.unsqueeze(1) - query_emb[i]
                    scores[i] = torch.max(self.gamma - torch.norm(distance, p=1, dim=-1), dim=-1)[0]
                return scores

            scores = torch.ones((batch_size, self.graph.n_entity), dtype=torch.float16).cuda()
            target_embeds = self.entity_embedding[:self.graph.n_entity]
            for i in range(batch_size):
                distance = target_embeds - query_emb[i].t().unsqueeze(0)
                scores[i] = self.gamma - torch.norm(distance, p=1, dim=-1)
            return scores
