...
def generate_id_neighbor(args, graph,
    ...
    # generate_id_neighbor_noise(heads, rels, tails, id_neighbor_num, dir, train_queries, train_answers, query_name_dict,
                            # 'train_queries_idneighbor_i.pkl')
    # generate_id_neighbor_noise(heads, rels, tails, id_neighbor_num, dir, valid_queries, valid_answers, query_name_dict,
    #                         'valid_queries_idneighbor_noise.pkl')
    generate_id_neighbor_noise(heads, rels, tails, id_neighbor_num, dir, test_queries, test_answers, query_name_dict,
                            'test_queries_idneighbor_noise_2p.pkl')

    # generate_id_neighbor_null(heads, rels, tails, id_neighbor_num, dir, train_queries, train_answers, query_name_dict,
    #                         'train_queries_idneighbor_null.pkl')
    # generate_id_neighbor_null(heads, rels, tails, id_neighbor_num, dir, valid_queries, valid_answers, query_name_dict,
    #                         'valid_queries_idneighbor_null.pkl')
    # generate_id_neighbor_null(heads, rels, tails, id_neighbor_num, dir, test_queries, test_answers, query_name_dict,
    #                         'test_queries_idneighbor_null.pkl')
    ...
    # load_helper_neighbor(heads, rels, tails, id_neighbor_num, dir, train_queries, train_answers, query_name_dict,
    #                      'train_helper.pkl')
    # load_helper_neighbor(heads, rels, tails, id_neighbor_num, dir, valid_queries, valid_answers, query_name_dict,
    #                      'valid_helper.pkl')
    # load_helper_neighbor(heads, rels, tails, id_neighbor_num, dir, test_queries, test_answers, query_name_dict,
    #                      'test_helper.pkl')
...
def generate_id_neighbor_helper(heads, rels, tails, id_neighbor_num, dir, queries, answers, query_name_dict, fname,
    ...
    for query_type in queries:
        ...
        for query in queries[query_type]:
            ...
            # elif query_name_dict[query_type] in ['2i', '3i']:
            ...
            #
            #         ansl = np.intersect1d(chosen_tails_1, chosen_tails_2)
            #
            #     else:
            #         chosen_heads_index_1 = np.squeeze(np.argwhere(heads == anchor[0]).T, axis=0)
            #         chosen_heads_index_2 = np.squeeze(np.argwhere(heads == anchor[1]).T, axis=0)
            ...
            #
            #         ansl = np.intersect1d(chosen_tails_1, chosen_tails_2, chosen_tails_3)
            #
            #     for i in range(len(query)):
            #         temp_map_inner = defaultdict(set)
            #         chosen_heads_index = np.squeeze(np.argwhere(heads == anchor[i]).T, axis=0)
            #         chosen_rels = rels[chosen_heads_index]
            #         chosen_tails = tails[chosen_heads_index]
            #
            #         for j in range(chosen_heads_index.shape[0]):
            #             if query not in
            #
            #             q_neighbor = (anchor[i], (chosen_rels[j],))
            #             a = chosen_tails[j]
            #             if q_neighbor == query[i] and a in answers[query]:
            #                 continue
            #             temp_map_inner[query[i]].add((anchor[i], chosen_rels[j], a))
            #
            #         temp_map_inner[query[i]] = random.sample(temp_map_inner[query[i]],
            #                                                  id_neighbor_num * 2
            #                                                  if len(temp_map_inner[query[i]]) > id_neighbor_num * 2
            #                                                  else len(temp_map_inner[query[i]]))
            #
            ...
            #     query_idn_dict[query_type].update(temp_map)
                ...
                query_idn_dict[query_type].update(temp_map)
            elif query_name_dict[query_type] == '2u-DNF' and 'con' in fname:
                temp_map = defaultdict(set)
                count += 1
                anchor = get_anchors(query_type, query)
                chosen_heads_index_1 = np.squeeze(np.argwhere(heads == anchor[0]).T, axis=0)
                ...
                if len(temp_map[query]) < int(id_neighbor_num / 3):
                    # print("list_2i", len(list_2i))
                    count_min += 1
            elif query_name_dict[query_type] == 'up-DNF' and 'con' in fname:
                temp_map = defaultdict(set)
                count += 1
                anchor = get_anchors(query_type, query)
                chosen_heads_index_1 = np.squeeze(np.argwhere(heads == anchor[0]).T, axis=0)
...
def generate_id_neighbor_noise(heads, rels, tails, id_neighbor_num, dir, queries, answers, query_name_dict, fname):
    query_idn_dict = defaultdict(dict)
    max_entity = np.max(heads) if np.max(heads) > np.max(tails) else np.max(tails)
    max_rel = np.max(rels)
    ...
    for query_type in queries:
        ...
        for query in queries[query_type]:
            ...
            if query_name_dict[query_type] == '1p':
                ...
                for i in range(id_neighbor_num * 10):
                    rel = random.randint(0, max_rel)
                    ans = random.randint(0, max_entity)
                    q_neighbor = (anchor, (rel, ))
                    if q_neighbor == query and ans in answers[query]:
                        continue
                    temp_map[query].add((anchor, rel, ans))
                ...
                anchor = get_anchors(query_type, query)[0]
                for i in range(id_neighbor_num * 10):
                    anc = random.randint(0, max_entity)
                    rel_1 = random.randint(0, max_rel)
                    ans_1 = random.randint(0, max_entity)
                    rel_2 = random.randint(0, max_rel)
                    ans_2 = random.randint(0, max_entity)

                    temp_map[query].add((anc, rel_1, ans_1, rel_2, ans_2))
                ...
                for i in range(id_neighbor_num * 10):
                    ...
                    temp_map[query].add((anchor, rel_1, ans_1,
                                         rel_2, ans_2,
                                         rel, ans))
                ...
                for i in range(100):
                    temp_map[query].add(((anchor[0], random.randint(0, max_rel), random.randint(0, max_entity)),
                                         (anchor[1], random.randint(0, max_rel), random.randint(0, max_entity))))
                ...
                for i in range(100):
                    temp_map[query].add(((anchor[0], random.randint(0, max_rel), random.randint(0, max_entity)),
                                         (anchor[1], random.randint(0, max_rel), random.randint(0, max_entity)),
                                         (anchor[2], random.randint(0, max_rel), random.randint(0, max_entity))))
                ...
                for i in range(id_neighbor_num * 10):
                    ...
                    rel1 = random.randint(0, max_rel)
                    rel2 = random.randint(0, max_rel)
                    rel3 = random.randint(0, max_rel)

                    q_neighbor = (((anchor[0], (rel1,)), (anchor[1], (rel2,))), (rel3, ))
                    a = random.randint(0, max_entity)
                    real_ans = random.randint(0, max_entity)
                    if q_neighbor == query and real_ans in answers[query]:
                        continue
                    temp_map[query].add((((anchor[0], rel1, a), (anchor[1], rel2, a)),
                                        (a, rel3, real_ans)))
                ...
                for i in range(id_neighbor_num * 10):
                    first_rel = random.randint(0, max_rel)
                    rel1 = random.randint(0, max_rel)
                    rel2 = random.randint(0, max_rel)
                    t = random.randint(0, max_entity)
                    a = random.randint(0, max_entity)
                    q_neighbor = ((anchor[0], (first_rel, rel1)), (anchor[1], (rel2,)))
                    if q_neighbor == query and a in answers[query]:
                        continue
                    temp_map[query].add(((anchor[0], first_rel, t, rel1, a), (anchor[1], rel2, a)))
...
def generate_id_neighbor_null(heads, rels, tails, id_neighbor_num, dir, queries, answers, query_name_dict, fname):
    query_idn_dict = defaultdict(dict)
    max_entity = np.max(heads) if np.max(heads) > np.max(tails) else np.max(tails)
    max_rel = np.max(rels)
...
class QueryDataset(Dataset):
    ...
    def collate_fn(self, idx_list):
        ...
        if self.model not in ['gqe', 'gqe-n', 'q2b', 'q2b-n',
                              ...
                              'gqe-distmult', 'gqe-complex', 'gqe-rotate', 'gqe-pairre']:
            subgraph = QueryDataset.get_subgraph(chosen_queries,
                                                 self.query_type,
                                                 self.sample_limit,
                                                 self.pre_sample,
    ...
    def get_subgraph_old(queries, query_type, sample_limit, pre_sample, query_neis_dict, del_ans_edges,
        ...
        def deal_sampler(nei_info, query, status):
            ...
            if status == 'train' and mp_model != 'basic':
                ...
                # if query_type in [(('e', ('r',)), ('e', ('r',))), (('e', ('r',)), ('e', ('r',)), ('e', ('r',)))]:
                #     helper_triple = []
                #     for subtriple in query:
                #         helper_triple.append({subtriple: train_idn_static[('e', ('r',))][subtriple]})
                ...
                #     helper_triple = []
                #     for subtriple in query:
                #         helper_triple.append({subtriple:
                #                                   train_idn_static[('e', ('r',))][subtriple] |
                #                                   valid_idn_static[('e', ('r',))][subtriple]})
            ...
            if nei_info is None:
                ...
                # if helper_triple is not None:
                #     count = 0
                #     for triple_dict in helper_triple:
                #         for subq, triple in triple_dict.items():
                #             node_id = get_all_nei_id(('e', ('r',)), triple, node_id)
                #             nei_edge_index, edge_id = extend_nei_edge_index(node_id, triple, nei_edge_index, query, edge_id, ('e', ('r',)), count)
                #             nei_edge_index = torch.tensor(nei_edge_index, dtype=torch.long, requires_grad=False)
                #         count += 1
                #     neiC_edge_index = nei_edge_index[:, neiA_edge_index.shape[1] + neiB_edge_index.shape[1]:]
                #     neiC_edge_type = torch.tensor(edge_id[len(neiA_edge_type) + len(neiB_edge_type):], dtype=torch.long)
