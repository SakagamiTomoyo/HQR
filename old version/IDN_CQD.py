...
class ProjectionAggregating(nn.Module):
    ...
    def forward(self, lhs, refer_embs, rel, refer_r=None, start_embs=None):
        ...
        # 237
        # global cur_type
        # if cur_type in ['2p', '2i', '3i']:
        #     const = 25
        # elif cur_type in ['3p', 'pi', 'ip', '2u-DNF']:
        #     const = 50
        # elif cur_type in ['1p']:
        #     const = 10
        # elif cur_type in ['up-DNF']:
        #     const = 100

        global cur_type
        if cur_type in ['1p', '2p']:
            const = 15
        elif cur_type in ['3p']:
            const = 50
        elif cur_type in ['2i', '3i']:
            const = 10
        elif cur_type in ['pi', 'ip', 'up-DNF', '2u-DNF']:
            const = 25
...
#         self.dim = dim
#
#         self.final_layer_1 = nn.Linear(self.dim * 4, self.dim * 2)
#         self.final_layer_2 = nn.Linear(self.dim * 2, self.dim * 2)
#
#         self.attn_layer_1 = nn.Linear(self.dim * 6, self.dim * 2)
#         self.attn_layer_2 = nn.Linear(self.dim * 2, self.dim * 2)
#
#         self.bias_layer_1 = nn.Linear(self.dim * 4, self.dim * 2)
#         self.bias_layer_2 = nn.Linear(self.dim * 2, self.dim * 2)
#
#         nn.init.xavier_uniform_(self.bias_layer_1.weight)
#         nn.init.xavier_uniform_(self.bias_layer_2.weight)
#
#         nn.init.xavier_uniform_(self.attn_layer_1.weight)
#         nn.init.xavier_uniform_(self.attn_layer_2.weight)
#         nn.init.xavier_uniform_(self.final_layer_1.weight)
#         nn.init.xavier_uniform_(self.final_layer_2.weight)
#
#     def forward(self, lhs, refer_embs, rel, refer_r=None, start_embs=None):
#
#         # 像ComplEx一样直接用得分来计算bias
#         # 可以考虑控制一下bias的大小
#
#         query_emb = [(lhs[0] * rel[0] - lhs[1] * rel[1]), (lhs[0] * rel[1] + lhs[1] * rel[0])]
#
...
#         refer_r = torch.cat(refer_r, dim=-1)
#
#         layer_1 = F.relu(self.attn_layer_1(torch.cat([bias, query_r, refer_r], dim=-1)))
#         layer_2 = torch.mean(self.attn_layer_2(layer_1) * bias, dim=-2)
#
#         ref = self.final_layer_2(F.relu(self.final_layer_1(torch.cat([query_emb, layer_2], dim=-1))))
#
#         # ref_norm = ref.norm(p=1, dim=-1).unsqueeze(1)
#         # query_norm = query_emb.norm(p=1, dim=-1).unsqueeze(1)
#         # const = 10  # add_best: 10   # nell 2 - 4 - 25
#         #
#         # ref = torch.where(ref == 0, ref, ref / (1e-9 + ref_norm / query_norm * const))
#
#         final_query_emb = query_emb + ref
#
#         return final_query_emb[:, :self.dim], final_query_emb[:, self.dim:]
